...
export const createModifierGroup = createAsyncThunk<
  ...
  { rejectValue: string }
>("modifier/createModifierGroup", async (payload, { rejectWithValue, dispatch }) => {
  try {
...
export const updateModifierGroup = createAsyncThunk<
  ...
  { rejectValue: string }
>("modifier/updateModifierGroup", async (payload, { rejectWithValue, dispatch }) => {
  try {
...
>("modifier/fetchModifiers", async (_, { rejectWithValue }) => {
  try {
    ...
    const message = error.response?.data?.message || "Failed to fetch modifiers";
    return rejectWithValue(message);
  }
});
...
>("modifier/createModifier", async (payload, { rejectWithValue, dispatch }) => {
  try {
    ...
    const message =
      error.response?.data?.message || "Failed to add modifier";
    toast.error(message);
    return rejectWithValue(message);
  }
});
...
const modifierSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(createModifier.fulfilled, (state, action: PayloadAction<Modifier>) => {
      ...
      .addCase(deleteModifier.fulfilled, (state, action: PayloadAction<string>) => {
